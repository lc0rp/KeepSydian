...
function parseCustomScopeInput(value: string): { iso?: string; error?: string } {
	...
	if (!trimmedValue) return { error: "Choose a custom date." };
	const match = trimmedValue.match(CUSTOM_SCOPE_INPUT_PATTERN);
	if (!match) return { error: "Choose a valid custom date." };
	const [, yearString, monthString, dayString, hourString, minuteString] = match;
	const year = Number(yearString), month = Number(monthString), day = Number(dayString), hour = Number(hourString), minute = Number(minuteString);
	const parsed = new Date(year, month - 1, day, hour, minute, 0, 0);
	if (Number.isNaN(parsed.getTime()) || parsed.getFullYear() !== year || parsed.getMonth() !== month - 1 || parsed.getDate() !== day || parsed.getHours() !== hour || parsed.getMinutes() !== minute) {
		return { error: "Choose a valid custom date." };
	}
	if (parsed.getTime() > Date.now()) return { error: "Custom date must be in the past." };
...
export class SyncProgressModal extends Modal {
	...
	private getDownloadScope(): DownloadScope {
		...
		if (this.downloadScopeKind === "custom-since") {
			...
			if (!parsed.iso) throw new Error(parsed.error ?? "Choose a custom date.");
			return { kind: "custom-since", since: parsed.iso };
		}
		return { kind: "last-sync" };
	}
	private getCustomScopeError(): string | null {
		return this.downloadScopeKind === "custom-since" ? parseCustomScopeInput(this.customSinceInput).error ?? null : null;
	}
	...
	private renderDownloadScopeSection(containerEl: HTMLElement) {
		const sectionEl = createChild(containerEl, "div"); sectionEl.classList.add("keepsidian-sync-center-mode-section", "keepsidian-sync-center-scope-section");
		createChild(sectionEl, "div", { text: "Start date" }).classList.add("keepsidian-sync-center-mode-label");
		const optionsEl = createChild(sectionEl, "div"); optionsEl.classList.add("keepsidian-sync-center-modes"); optionsEl.setAttribute("role", "radiogroup"); optionsEl.setAttribute("aria-label", "Start date");
		const lastSuccessfulDownloadDate = this.getLastSuccessfulDownloadDate();
		this.renderDownloadScopeOption(optionsEl, "Last successful sync", "last-sync", lastSuccessfulDownloadDate ? `Last sync: ${formatScopeTimestamp(lastSuccessfulDownloadDate)}.` : "None yet.");
		this.renderDownloadScopeOption(optionsEl, "All dates", "all", ""); this.renderDownloadScopeOption(optionsEl, "Custom", "custom-since", "");
		if (this.downloadScopeKind === "custom-since") {
			const inputWrap = createChild(sectionEl, "label"); inputWrap.classList.add("keepsidian-sync-center-scope-input-wrap");
			const input = createChild(inputWrap, "input"); input.type = "text"; input.value = this.customSinceInput; input.placeholder = CUSTOM_SCOPE_INPUT_FORMAT; input.autocomplete = "off";
			input.setAttribute("aria-label", `Custom start date (${CUSTOM_SCOPE_INPUT_FORMAT})`); input.setAttribute("data-keepsidian-role", "custom-since-input"); input.classList.add("keepsidian-sync-center-scope-input");
			const helper = createChild(sectionEl, "div"); helper.classList.add("keepsidian-sync-center-scope-helper");
			const syncCustomScopeHelper = () => {
				const error = parseCustomScopeInput(input.value).error ?? null;
				helper.textContent = error ?? `Use ${CUSTOM_SCOPE_INPUT_FORMAT}. Notes changed after this date will be included.`; helper.classList.toggle("is-warning", Boolean(error));
			};
			syncCustomScopeHelper();
			input.addEventListener("input", () => { this.customSinceInput = input.value; this.modalAlert = null; syncCustomScopeHelper(); });
			input.addEventListener("change", () => { this.customSinceInput = input.value; this.modalAlert = null; syncCustomScopeHelper(); void this.refreshUI(); });
		}
	}
	private renderDownloadScopeOption(containerEl: HTMLElement, label: string, kind: DownloadScopeKind, description: string) {
		const button = this.createActionButton(containerEl, "", async () => { this.setDownloadScopeKind(kind); });
		button.classList.add("keepsidian-sync-center-mode-button", "keepsidian-sync-center-scope-button"); button.classList.toggle("is-selected", this.downloadScopeKind === kind);
		button.setAttribute("role", "radio"); button.setAttribute("aria-checked", this.downloadScopeKind === kind ? "true" : "false");
		const indicator = createChild(button, "span"); indicator.classList.add("keepsidian-sync-center-mode-indicator"); indicator.setAttribute("aria-hidden", "true");
		const body = createChild(button, "span"); body.classList.add("keepsidian-sync-center-scope-option-body");
		createChild(body, "span", { text: label }).classList.add("keepsidian-sync-center-mode-text");
		createChild(body, "span", { text: description }).classList.add("keepsidian-sync-center-scope-option-copy");
	}
